...
func initializeNode(g gameState, tree *Tree) *node {
	return &node{
		state:          g,
		tree:           tree,
		nodeScore:      make(map[Game.Player]float64),
...
func (n *node) runSimulation() ([]Game.Player, float64) {
	...
	if terminalState {
		...
		winners = n.children[selectedChildIndex].defaultPolicy()
		scoreToAdd = 1.0 / float64(len(winners))
...
func (n *node) defaultPolicy() []Game.Player {
